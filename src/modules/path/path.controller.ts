...
export class PathController {
  constructor(
    @InjectRepository(Path) private readonly pathRepository: EntityRepository<Path>,
  ) { }

  @Get()
  @ApiOperation({ summary: 'List all paths' })
  @ApiResponse({ status: 200, description: 'Found paths', type: [PathResponseDto] })
  async find() {
  ...
  @ApiResponse({ status: 200, description: 'The found path', type: PathResponseDto })
  @ApiResponse({ status: 404, description: 'Path not found' })
  async findOne(@Param('id', ParseIntPipe) id: number) {
    const path = await this.pathRepository.findOne({ pathId: id });

    if (!path) {
      throw new HttpException(`Path with ID ${id} not found`, HttpStatus.NOT_FOUND);
    }
