...
export class RelicSetController {
  constructor(
    @InjectRepository(RelicSet) private readonly relicSetRepository: EntityRepository<RelicSet>,
  ) { }

  @Get()
  @ApiOperation({ summary: 'List all relic sets' })
  @ApiResponse({ status: 200, description: 'Found relic sets', type: [RelicSetResponseDto] })
  async find() {
  ...
  @ApiResponse({ status: 200, description: 'The found relic set', type: RelicSetResponseDto })
  ...
  async findOne(@Param('id', ParseIntPipe) id: number) {
    ...
    if (!relicSet) {
      throw new HttpException(`Relic set with ID ${id} not found`, HttpStatus.NOT_FOUND);
    }
