...
export class LightconeController {
  constructor(
    @InjectRepository(Lightcone)
    private readonly lightconeRepository: EntityRepository<Lightcone>,
  ) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'Found lightcones',
    type: [LightconeResponseDto],
  })
  async find() {
  ...
  @ApiResponse({
    status: 200,
    description: 'The found lightcone',
    type: LightconeResponseDto,
  })
  ...
  async findOne(@Param('id', ParseIntPipe) id: number) {
    ...
    if (!lightcone) {
      throw new HttpException(
        `Lightcone with ID ${id} not found`,
